...
export default function CarouselIndicator(props: Props): JSX.Element {
  return (
    ...
    <div className="absolute bottom-[6.2%] left-1/2 z-5 flex h-[9.5px] -translate-x-1/2 gap-[3.75px]">
      {/* count만큼 반복해서 점(●) 생성 */}
      {Array.from({ length: props.count }).map(function (_, index: number): JSX.Element {
        return (
          <span
            ...
            className={`h-[9.5px] w-[9.5px] rounded-full ${index === props.activeIndex ? "bg-black" : "bg-white"}`}
          />
        );
      })}
    </div>
