...
import { addShippingAddress, AddShippingAddressRequest, addShippingAddressRequest } from "@/lib/api/shipping-address";
...
function AddShippingAddressModal() {
  const close = useAddShippingAddressModalStore((state) => state.close);
  const form = useForm({
    resolver: valibotResolver(addShippingAddressRequest),
  });
  ...
  const handleSubmit: SubmitHandler<AddShippingAddressRequest> = async (input) => {
    try {
  ...
  return (
    <Modal size="full">
      <ModalWrap>
        <ShippingAddressFormHeader title="배송지 등록" onClickClose={close} />
        <Title />
